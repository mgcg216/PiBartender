...
class Bar:
                ...
                if self.pour == True:
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            ...
                            time.sleep(10*pause)
                            d.drink_option(drinks_dic[drinks_arr[self.position]])
                            time.sleep(pause)
                            self.dispense = False
                            time.sleep(pause)
                    if event.type == pygame.KEYDOWN:
    ...
    def dispense_window(self):
        ...
        #A Dictionary "associative arrary" that will 
        self.screen.fill(BGCOLOR)
...
while b.running:
    #Buttons
    """ Problem RuntimeError: Please set pin numberin mode using GPIO.setmode(BPIO.BOARD)
    I think this is due to the Drinks() class mess with GPIO pin stuff. Look into after Drinks works"""
